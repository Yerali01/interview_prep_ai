...
export async function runCompleteMigration(): Promise<CompleteMigrationResult> {
  ...
  const results: CompleteMigrationResult = {
    ...
    details: {
      ...
      projectTechnologies: { success: false, migrated: 0, errors: [], skipped: 0 },
  ...
  try {
    ...
    // Calculate totals
    results.totalMigrated = Object.values(results.details).reduce((sum, result) => sum + result.migrated, 0)
    results.totalErrors = Object.values(results.details).reduce((sum, result) => sum + result.errors.length, 0)
...
async function migrateTopics(): Promise<MigrationResult> {
  const result: MigrationResult = { success: false, migrated: 0, errors: [], skipped: 0 }

  try {
    // Get existing Firebase topics to avoid duplicates
    const existingTopics = await getDocs(collection(db, "topics"))
    const existingSlugs = new Set(existingTopics.docs.map((doc) => doc.data().slug))
    ...
    result.success = true
    console.log(`✅ Topics migration completed: ${result.migrated} migrated, ${result.skipped} skipped`)
  } catch (error: any) {
...
async function migrateDefinitions(): Promise<MigrationResult> {
  ...
  try {
    ...
    const existingTerms = new Set(existingDefinitions.docs.map((doc) => doc.data().term))
    ...
    result.success = true
    console.log(`✅ Definitions migration completed: ${result.migrated} migrated, ${result.skipped} skipped`)
  } catch (error: any) {
...
async function migrateProjectsComplete(): Promise<{
  ...
  const projectsResult: MigrationResult = { success: false, migrated: 0, errors: [], skipped: 0 }
  const technologiesResult: MigrationResult = { success: false, migrated: 0, errors: [], skipped: 0 }
  const featuresResult: MigrationResult = { success: false, migrated: 0, errors: [], skipped: 0 }

  try {
    // Get existing Firebase projects to avoid duplicates
    const existingProjects = await getDocs(collection(db, "projects"))
    const existingSlugs = new Set(existingProjects.docs.map((doc) => doc.data().slug))
    ...
    console.log(`✅ Projects migration completed: ${projectsResult.migrated} projects migrated`)
    console.log(`✅ Technologies migration completed: ${technologiesResult.migrated} technologies migrated`)
    console.log(`✅ Features migration completed: ${featuresResult.migrated} features migrated`)
  } catch (error: any) {
...
async function migrateQuizzesComplete(): Promise<{
  ...
  const quizzesResult: MigrationResult = { success: false, migrated: 0, errors: [], skipped: 0 }
  const questionsResult: MigrationResult = { success: false, migrated: 0, errors: [], skipped: 0 }

  try {
    // Get existing Firebase quizzes to avoid duplicates
    const existingQuizzes = await getDocs(collection(db, "quizzes"))
    const existingSlugs = new Set(existingQuizzes.docs.map((doc) => doc.data().slug))
    ...
    console.log(`✅ Quizzes migration completed: ${quizzesResult.migrated} quizzes migrated`)
    console.log(`✅ Questions migration completed: ${questionsResult.migrated} questions migrated`)
  } catch (error: any) {
...
export async function clearFirebaseData() {
  ...
  for (const collectionName of collections) {
    try {
      ...
      if (batchCount > 0) {
        await batch.commit()
        console.log(`🗑️ Cleared ${batchCount} documents from ${collectionName}`)
      }
    } catch (error) {
