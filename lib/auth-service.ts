...
function handleAuthError(error: any): { message: string; code?: string } {
  ...
  const errorMessages: Record<string, string> = {
    ...
    "auth/credential-already-in-use": "This GitHub account is already linked to another user.",
  }
...
export async function signInWithEmail(email: string, password: string): Promise<AuthResult> {
  try {
    ...
    try {
      ...
      console.warn("⚠️ Failed to fetch user data from Firestore:", firestoreError)
    }
...
// Sign out
export async function signOutUser(): Promise<{ error: { message: string; code?: string } | null }> {
  try {
...
// Reset password
export async function resetPassword(email: string): Promise<{ error: { message: string; code?: string } | null }> {
  try {
