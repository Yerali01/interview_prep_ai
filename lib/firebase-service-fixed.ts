...
export async function firebaseGetTopics(): Promise<Topic[]> {
  try {
    ...
    // Sort topics by difficulty level
    const difficultyOrder: Record<DifficultyLevel, number> = {
      beginner: 1,
      intermediate: 2,
      advanced: 3,
    };
    topics.sort((a, b) => difficultyOrder[a.level] - difficultyOrder[b.level]);

    console.log(
      `✅ Successfully fetched ${topics.length} topics from Firebase`
    );
    return topics;
  } catch (error: any) {
...
export async function firebaseGetProjects(): Promise<Project[]> {
  try {
    ...
    console.log(
      `✅ Successfully fetched ${projects.length} projects from Firebase`
    );
    return projects;
  } catch (error: any) {
